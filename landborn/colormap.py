...
def plot_colormap_gradient(colormap_name):
...
def plot_colormap_in_rgb_space(colormap_name, num_samples=500):
    if not isinstance(colormap_name, str):
        title = colormap_name.name
    else:
        title = colormap_name
...
def create_custom_colormap(num_points=256, title="MyColormap"):
...
def visualize_colormap(colormap):
    gradient = np.linspace(0, 1, 256)
    gradient = np.vstack((gradient, gradient))
...
def delta_e(colormap_name, num_points=256):
...
def delta_e_lightness(colormap_name, num_points=256):
    if isinstance(colormap_name, str):
        cmap = plt.get_cmap(colormap_name)
        title = colormap_name
    else:
...
def compare_colormaps(cmap1, cmap2):
...
if __name__ == "__main__":
    ...
    compare_colormaps(custom_cmap, deuter_colormap)
    delta_e(custom_cmap)
